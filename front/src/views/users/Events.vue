      ...
      <ui5-card class="full-card">
        ...
        <div v-if="loader" style="width: 50%; margin: 0 auto">
          <ui5-message-strip design="Warning" hideCloseButton>
            <img src="/src/assets/loading.gif" alt="Подождите" width="16" height="16" slot="icon" />
            Пожалуйста, подождите...
          </ui5-message-strip>
        </div>
        <EventsGrid v-if="(events.length > 0) && (!(loader))"
                    v-bind:showDetailsFunction="showEventDetails"
                    v-bind:eventsList="events" />
      </ui5-card>
    </slot>
  </LkBase>
  <ui5-dialog ref="eventDetailDialog" class="add-edit-dialog" draggable>
    <ui5-bar slot="header">
    ...
    <div v-if="!(selectedEvent.loading)">
      ...
      <div class="detail-event-info-div" style="width: 100%;">
        <div v-if="selectedEvent.tab === 'info'" style="max-width: 100%" v-html="selectedEvent.info" />
...
export default {
  ...
  methods: {
    ...
    async getEventsByType(event_type) {
      this.loader = true
      apiRequest(
          '/api/v1/users/events/?event_type='+event_type,
          'GET',
          true,
          null,
          false,
    ...
    async checkAppExists() {
      apiRequest(
          '/api/v1/users/apps/check_exist/'+this.selectedEvent.id+'/',
          'GET',
          true,
          null,
          false,
    ...
    async getEventInfo() {
      apiRequest(
          '/api/v1/users/event_info/'+this.selectedEvent.id+'/',
          'GET',
          true,
          null,
          false,
    ...
    async eventAppCreate() {
      ...
      apiRequest(
          '/api/v1/users/apps/app_submit/'+this.selectedEvent.id+'/',
          'GET',
          true,
          null,
          false,
